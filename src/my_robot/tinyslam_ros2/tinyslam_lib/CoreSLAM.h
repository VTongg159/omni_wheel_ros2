/*
 * CoreSLAM.h - TinySLAM core algorithm header
 * Source: https://github.com/OpenSLAM-org/openslam_tinyslam
 * Kept verbatim - do NOT modify this file.
 */
#ifndef _TINYSLAM_H_
#define _TINYSLAM_H_
...
#define TS_MAP_SCALE   0.1        /* meters per pixel */
...
void ts_map_update(ts_scan_t *scan, ts_map_t *map, ts_position_t *position,
                   int quality, int hole_width);
...
ts_position_t ts_monte_carlo_search(ts_randomizer_t *randomizer,
                                    ...
                                    int stop, int *bestdist);

/* Extensions */
double ts_distance(ts_position_t *pos1, ts_position_t *pos2);
void   ts_save_map_pgm(ts_map_t *map, ts_map_t *overlay, char *filename,
                       int width, int height);
void   ts_draw_scan(ts_scan_t *scan, ts_map_t *map, ts_position_t *pos);
void   ts_draw_scan_RGB(ts_scan_t *scan, ts_map_t *map, ts_position_t *pos,
                        unsigned char *pixmap, int scale, int reversey);

typedef struct {
    double r;     /* wheel radius  */
    double R;     /* half axis length */
    int    inc;   /* encoder increments/turn */
    double ratio; /* left/right ratio */
} ts_robot_parameters_t;

typedef struct {
    double offset;               /* laser offset from rotation center */
    int    scan_size;            /* number of points per scan */
    int    angle_min;            /* start angle (deg) */
    int    angle_max;            /* end angle  (deg) */
    int    detection_margin;     /* first element to use */
    double distance_no_detection;/* value when laser returns 0 */
...
/* Loop closing */
ts_position_t ts_close_loop_position(ts_state_t *state,
