...
void SlamGmapping::init() {
    ...
    double map_interval = this->declare_parameter<double>("map_update_interval", 5.0);
    map_update_interval_ = tf2::durationFromSec(map_interval);

    last_map_update = tf2::TimePointZero;

    // Các tham số thuật toán GMapping
    maxUrange_ = this->declare_parameter<double>("maxUrange", 4.0);
    maxRange_ = this->declare_parameter<double>("maxRange", 6.0);
    minimum_score_ = this->declare_parameter<double>("minimum_score", 0.0);
...
void SlamGmapping::startLiveSlam() {
    ...
    sst_ = this->create_publisher<nav_msgs::msg::OccupancyGrid>("map", rclcpp::QoS(rclcpp::KeepLast(1)).transient_local());
    sstm_ = this->create_publisher<nav_msgs::msg::MapMetaData>("map_metadata", rclcpp::QoS(rclcpp::KeepLast(1)).transient_local());

    // FIX #6: Sử dụng shared_from_this() thay vì null-deleter
    ...
    scan_filter_sub_ = std::make_shared<message_filters::Subscriber<sensor_msgs::msg::LaserScan>>(
        node_ptr, "scan", rclcpp::SensorDataQoS().get_rmw_qos_profile());

    scan_filter_ = std::make_shared<tf2_ros::MessageFilter<sensor_msgs::msg::LaserScan>>(
        *scan_filter_sub_, *buffer_, odom_frame_, 10, node_ptr);
...
// FIX #1: Sửa lỗi transform() - loại bỏ tham số thứ 4 không hợp lệ
bool SlamGmapping::getOdomPose(GMapping::OrientedPoint& gmap_pose, const rclcpp::Time& t) {
    ...
    try {
        // Transform không cần timeout tham số thứ 4, chỉ dùng 3 tham số
...
// FIX #2: Sửa lỗi transform() với timeout đúng cách
bool SlamGmapping::initMapper(const sensor_msgs::msg::LaserScan::ConstSharedPtr scan)
{
    ...
    try{
        ...
        // FIX #2a: Thêm timeout parameter đúng cách
        buffer_->transform(ident, laser_pose, base_frame_, tf2::durationFromSec(1.0));
    }
    catch (tf2::TransformException& e){
    ...
    try
    {
        // FIX #2b: Thêm timeout parameter
        buffer_->transform(up, up, laser_frame_, tf2::durationFromSec(1.0));
    }
    catch(tf2::TransformException& e)
    {
...
// FIX #5: Xử lý memory an toàn hơn cho RangeReading
bool SlamGmapping::addScan(const sensor_msgs::msg::LaserScan::ConstSharedPtr scan, GMapping::OrientedPoint& gmap_pose) {
    ...
    std::unique_ptr<double[]> ranges_double(new double[scan->ranges.size()]);
    ...
    // unique_ptr sẽ tự động delete khi ra khỏi scope
...
// FIX #4: Thêm lock_guard để tránh race condition
void SlamGmapping::laserCallback(sensor_msgs::msg::LaserScan::ConstSharedPtr scan) {
    ...
    if(addScan(scan, odom_pose))
    {
        ...
        // FIX #4: Sử dụng lock_guard thay vì manual lock
        {
...
void SlamGmapping::updateMap(const sensor_msgs::msg::LaserScan::ConstSharedPtr scan)
{
    ...
    // FIX #4: Sử dụng lock_guard để tránh deadlock
...
// FIX #3: Sửa publishTransform với lock_guard thay vì manual lock/unlock
